...
import { requireProjectId, confirmAction, requireProductIdArg } from '../helpers';
...
export function register(program: Command): void {
    ...
    .action(async (productId: string | undefined, opts) => {
      ...
      if (!opts.force) {
        const confirmed = await confirmAction(
          `Are you sure you want to delete product ${requiredProductId}?`,
        );
        if (!confirmed) {
      ...
      await api.del(`/projects/${pid}/products/${requiredProductId}`);
      output(null, () => printSuccess(`Product ${requiredProductId} deleted.`));
    });
  ...
  cmd
    ...
    .action(async (productId: string | undefined, opts) => {
      const requiredProductId = requireProductIdArg(productId);
      const pid = requireProjectId(opts);
      const data = await api.post(`/projects/${pid}/products/${requiredProductId}/actions/archive`);
      output(data, () => printSuccess(`Product ${requiredProductId} archived.`));
    });
  ...
  cmd
    ...
    .action(async (productId: string | undefined, opts) => {
      const requiredProductId = requireProductIdArg(productId);
      const pid = requireProjectId(opts);
      const data = await api.post(`/projects/${pid}/products/${requiredProductId}/actions/unarchive`);
      output(data, () => printSuccess(`Product ${requiredProductId} unarchived.`));
    });
}
